...
{	
	vTask_Delay(SNAKE_SPEED);//非阻塞延时
	
	static uint8_t SnakeInit_Flag = 0; //Snake初始化状态标志位
	uint8_t key;	//存按键值
	
	if(SnakeInit_Flag == 0)//进来只初始化一次
	{
		//绘制游戏边框
		CreateMap();
		//初始化游戏
	...
	if(IR_GetKey(&key) == 1)//有按键
	{
		switch(key)
		{
			...
			case Key_A:
			case Key_D:
				Snake_P1.Direction = (DIRECTION)key;
				break;
