...
void Alarm_Check(void)
{
	...
	if(IR_GetKey(&key) == 1 && key == Key_JingHao && Alarm_setHandle.alarm_clock[3] == 2)
	{
		Buzzer_SetMode(&PB14_Buzzer, Buzzer_OFF, 0, 0); //关闭蜂鸣器
		vTask_Delete(Alarm_Check);
		Alarm_setHandle.alarm_clock[3] = 0;
		return;
	}
	...
	{			
		if(SetAlarm_arr[3] == 1)//当前闹钟处于开启状态
		{
			SetAlarm_arr[3] = 2;  //当前闹钟处于蜂鸣状态
...
static void Alarm_Set(void)
{
	...
	if(SetAlarm_arr[3] == 1 || SetAlarm_arr[3] == 2) OLED_Printf(0, 48, OLED_8X16, "Alarm ON");
	...
	switch(key)
	{
		...
		case Key_OK:
			...
			if(SetAlarm_arr[Alarm_index] == 0) 
			{
				...
				Scheduler_AddTask(Alarm_Check, 250, 6, 500); //创建时间检测任务
			}
			else if(SetAlarm_arr[Alarm_index] == 1 || SetAlarm_arr[Alarm_index] == 2)
			{
...
{
	Alarm_menu_state *Buzzer = &Alarm_setHandle;  //句柄
	static uint8_t Buzzer_index =0; //0-1表示选项
	uint8_t key;	//存按键值
	
	//显示模块
	OLED_Clear();
	OLED_Printf(0, 0, OLED_8X16, "Buzzer-Config");
	OLED_Printf(0, 16, OLED_8X16, "Buzzer_ON:%d",  Buzzer->buzzer_ontime);
	OLED_Printf(0, 32, OLED_8X16, "Buzzer_OFF:%d", Buzzer->buzzer_offtime);
	OLED_ReverseArea(0, 16 * Buzzer_index + 16, 128, 16);
	...
	switch(key)
	{
		...
		case Key_S:
			if(Buzzer_index == 1) Buzzer_index = 0;
			Buzzer_index++;
			break;
		...
		case Key_JingHao:
			if(Buzzer_index == 0) Buzzer->buzzer_ontime = 0;
			if(Buzzer_index == 1) Buzzer->buzzer_offtime = 0;
			break;
		
		default:
			if(key > 9) break;//不是数字键退出
			if(Buzzer_index == 0) Buzzer->buzzer_ontime = Buzzer->buzzer_ontime * 10 + key; 
			if(Buzzer_index == 1) Buzzer->buzzer_offtime = Buzzer->buzzer_offtime * 10 + key; 
	}
	//更新蜂鸣器状态
	if(Buzzer->buzzer_ontime >= 200 && Buzzer->buzzer_offtime >= 200)
	{
		Buzzer_SetMode(&PB14_Buzzer, Buzzer_Beep, Buzzer->buzzer_ontime, Buzzer->buzzer_offtime);
	}
	else if(Buzzer->buzzer_ontime < 200 && Buzzer->buzzer_offtime >= 200)//蜂鸣时间小于200关闭
	{
		Buzzer_SetMode(&PB14_Buzzer, Buzzer_OFF, Buzzer->buzzer_ontime, Buzzer->buzzer_offtime);
	}
	else if(Buzzer->buzzer_ontime >= 200 && Buzzer->buzzer_offtime < 200)//不蜂鸣时间小于200开启
	{
		Buzzer_SetMode(&PB14_Buzzer, Buzzer_ON, Buzzer->buzzer_ontime, Buzzer->buzzer_offtime);
	}
	else
	{
		Buzzer_SetMode(&PB14_Buzzer, Buzzer_OFF, 0, 0);
	}
