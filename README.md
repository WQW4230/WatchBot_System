...
项目简介
本仓库是一个基于 STM32F103C8T6（主控）与 ESP32-S3-CAM（视觉）构建的模块化双 MCU 嵌入式平台。平台集成自研轻量级调度系统 FunOS、机械臂控制、红外菜单与 OLED与TFT 界面，并实现人脸识别追踪报警与实时画面传输。整体设计面向工程可用：模块化、可配置、易扩展的小型项目
...
* 使用 UART TTL 通信，命令集为文本或二进制帧（含帧头、长度校验、命令ID、数据、帧尾）。
